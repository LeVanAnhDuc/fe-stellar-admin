...
function ModalInsert({ handleClose, show, PostUtilities }) {
    ...
    const handelCreateUtilities = async () => {
        await utilitiesApi
            .postUtilities(formData)
            .then((res) => {
        ...
        <>
            <Modal show={show} onHide={handleClose} backdrop="static" size="lg" centered className={cx('modal')}>
                <Modal.Header closeButton className={cx('header')}>
                    <Modal.Title className={cx('title')}>THÊM TIỆN ÍCH</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
                ...
                <Modal.Footer>
                    ...
                    <Button className={cx('btn')} filled_1 onClick={handelCreateUtilities}>
                        Xác nhận
                    </Button>
                </Modal.Footer>
            </Modal>
